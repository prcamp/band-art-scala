...
case class ParticleScene(parent: Main) extends ShapeField(parent){
  var numShapes = 1000
  var initposField0 = (0 to numShapes).map(
...
class ParticleShape(parent: Main, override val initpos: Point2d) extends ShapeObj(parent,new PShape()) {
  val refradius = 10
  var radius = 5
